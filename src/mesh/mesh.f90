...
IF(MeshMode .GT. 0)THEN
  CALL Connect(reconnect=.FALSE.)                           ! Create connection between elements
  IF(useCurveds.AND.Logging) CALL CountSplines()  ! In case of restart there can be splines
END IF
CALL buildEdges()
...
IF(MeshPostDeform.NE.0)THEN
  ...
  CASE(1) !reconnect all sides
    CALL Connect(reconnect=.TRUE.)                           ! Create connection between elements
  CASE(2) !reconnect all sides, overwrite periodic connections 
    DO i=1,nVV
      vv(:,i)=postVV(:,i)
    END DO
    CALL Connect(reconnect=.TRUE.)                           ! Create connection between elements
